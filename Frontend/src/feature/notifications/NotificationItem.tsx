...
export const NotificationItem: React.FC<NotificationItemProps> = ({
  ...
  return (
    <div
      ...
      className={`flex items-start gap-4 border-b border-border-primary px-4 py-4 transition-colors cursor-pointer hover:bg-bg-secondary/40 ${
        !isRead
          ? "bg-accent-primary/5 relative after:absolute after:left-0 after:top-0 after:bottom-0 after:w-0.5 after:bg-accent-primary"
          : ""
      }`}
    >
      <div className="mt-1 shrink-0 rounded-full p-1.5 bg-bg-secondary border border-border-primary">
        {getIcon()}
      </div>
      ...
      <div className="flex flex-1 gap-3 min-w-0">
        ...
        <div className="flex flex-col min-w-0">
          <p className="text-sm text-text-secondary leading-normal">
            ...
            <span className="text-text-muted">{getActivityText()}</span>
          </p>
          <span className="mt-1 text-[11px] text-text-muted">{createdAt}</span>
        </div>
      </div>
    </div>
  );
